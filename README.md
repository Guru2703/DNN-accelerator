...
Viewed 10 files, ran a command, edited a file · 1 note
Viewed 10 files, ran a command, edited a file · 1 note
I've appended three new sections (26–28) to the same README, based directly on the RTL you uploaded:

§26 — What Is a Systolic Array: explains the general concept, then grounds it in your actual PE.sv (its two accumulation modes — normal MAC vs. the byte-injection trick used for bias) and control_unit.sv's diagonal wavefront pe_en/w_shift pattern (fill/drain over ~34 cycles). Left an image placeholder images/systolic_array.gif for the GIF you'll add.
§27 — Other Important Hardware Blocks: one paragraph per remaining module — PC, Instr_Mem, Instr_Decoder, control_unit, TEST_Mem, Shift_Reg_1x16, scale, LUT_AF — describing what each does and how it ties back to the earlier math/software sections. Left a placeholder images/architecture.png for the architecture diagram you'll add.
§28 — Instruction Set: a bit-field diagram of the 64-bit instruction word (opcode/shift/smode/address), followed by the table you asked for — instruction name, opcode in binary and hex, and a plain-English description of what it does (LOAD_INPUT, LOAD_WEIGHT, MAC_ACC, MAC_RESET, SCALE, ACTIVATE) — derived directly from Instr_Decoder.sv's case(instr[63:61]) and cross-checked against the instruction strings your sub_mat_mul function actually emits.
One honest caveat: I didn't have SA.sv (the systolic array top-level file itself) uploaded — only PE.sv and how Coprocessor.sv instantiates SA. So the description of the array's internal wiring is inferred from the PE's ports and the control unit's enable patterns rather than read directly from SA.sv. If you upload that file, I can tighten up the exact interconnect description.


Readme
Document · MD 
now add in that readme how that software part generate instruction for hardware from each code to compute a inference and result part i will write after this


Ran 3 commands
Ran 3 commands
I'll append a new section detailing exactly how sub_mat_mul and the rest of the software pipeline construct each 64-bit instruction line, tying it directly to the opcode table.


Edited a file, read a file
Edited a file, read a file
Added Section 29 — How the Software Generates Hardware Instructions, placed right after the instruction set table. It walks through Matrix.sub_mat_mul exactly as coded in your notebook and ties every piece back to the RTL:

Step 1: the once-per-layer SCALE/ACTIVATE setup instructions and why they're only emitted once (single-tile FNN layers).
Step 2: the per-(j,k) LOAD_INPUT / LOAD_WEIGHT / MAC_ACC/MAC_RESET triplet, including the exact address arithmetic (4*k, 64*k + 64*c_a*j) and why reset = str(int(not k)) maps to opcode 011 on the first reduction step and 010 afterward.
Step 3: the four bias byte-plane injection triplets, showing that shift = l*8 produces exactly the four values PE.sv checks, and that the identity matrix i_mat is reused as the "weight" operand to pass bias bytes through unmodified.
Step 4: a flow diagram showing how NN.quantize_forward orchestrates initiate_addr (one-time memory allocation → memory1.mem) and sub_mat_mul calls across all layers, concatenating their instructions into the single straight-line program written to instr_mem.mem.
This is placed right before where you said you'll add the results section — ready for you to continue from here.


Readme
Document · MD 

# DNN-accelerator # Mathematical Formulation and Matrix Tiling A Fully Connected (FC) layer in a neural network can be represented as a matrix multiplication followed by bias addition and an activation function. This representation allows the computation of a DNN layer to be mapped onto the matrix

pasted

correct the syntax in under result topic and make sure each pic and mp4 file should be visible with proper heading and png files are visible but not the mp4 one correct them and share the complete final readme


Ran 2 commands, created a file
Ran 2 commands, created a file

check whether Result section already exists in output file

clear old readme to rebuild from user's current source of truth

Final complete README with mp4 embed fix and cleaned-up Result section

You are out of free messages(opens in new tab) until 9:40 PM
Upgrade
...
A 16-deep, 8-bit-wide shift register used both for the single activation-input lane and, instantiated 16 times, for the 16 weight lanes feeding the systolic array. It supports two operations: a parallel `en`-driven load of 16 new bytes (4 packed 32-bit words) from `TEST_Mem`, and a serial `shift`-driven walk that advances existing contents by one position per cycle — this is what actually streams activations or weights into the array one byte per cycle, in sync with the wavefront pattern from `control_unit`.
...
| **LOAD_WEIGHT** | `001` | `0x1` | Loads/shifts new INT8 weight bytes into the 16 weight shift registers (`mode = 1`). |
...
## 30. Verification Methodology
 
The `instr_gen_and_verification_DNN.ipynb` notebook is used not only to generate the instruction stream and memory image consumed by the hardware, but also to verify the hardware simulation's results for correctness. Because the Python model performs the exact same sequence of operations as the RTL — tiled INT8×INT8 multiply, INT32 accumulation, bias injection, requantization, and LUT-based activation (Sections 19 and 29) — its output is expected to match the hardware simulation's output bit-for-bit. Any mismatch between the two therefore points to a memory- or instruction-generation bug rather than a numerical or algorithmic error, since the underlying computation itself has already been validated in software.
 
## 31. MNIST Handwritten Digit Recognition — Accuracy
 
The accelerator was validated end-to-end on an MNIST handwritten-digit-recognition model, comparing the original FP32 model, the INT8 post-training-quantized (PTQ) software model, and the hardware simulation output.
 
| Model | Accuracy |
|---|---|
| FP32 (floating-point baseline) | 96.43% |
| INT8 PTQ (quantized, hardware-accurate) | 95.85% |
| Accuracy retained (INT8 relative to FP32) | 99.48% |
 
**Hardware Simulation Output**
 
![Hardware Output](images/hardware_output.png)
 
**Software (Python Model) Output**
 
![Software Output](images/python_output.png)
 
The hardware and software outputs match, confirming that the instruction stream and memory image generated by the notebook (Sections 24 and 29) drive the RTL design (Sections 26–28) to produce the same quantized predictions as the reference Python model.
